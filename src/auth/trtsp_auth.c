...
/**@defgroup trtsp_auth_group RTSP basic/digest authentication (RFC 2617)
*/

/**@ingroup trtsp_auth_group
 *
 * Generates RTSP-basic response as per RFC 2617.
 *
 ...
 **/
tsk_size_t trtsp_auth_basic_response(const char* userid, const char* password, char** response)
{
    tsk_size_t ret;
 ...
 **/
int trtsp_auth_digest_HA1(const char* username, const char* realm, const char* password, tsk_md5string_t* ha1)
{
    int ret;
 ...
 * @return	Zero if succeed and non-zero error code otherwise.
 **/
int trtsp_auth_digest_HA1sess(const char* username, const char* realm, const char* password, const char* nonce, const char* cnonce, tsk_md5string_t* ha1sess)
{
    int ret;
 ...
 *
 *
 * @param [in,out]	method		The RTSP/SIP method name.
 * @param [in,out]	url			The RTSP URL or SIP URI of the request.
 ...
 * @return	Zero if succeed and non-zero error code otherwise.
 **/
int trtsp_auth_digest_HA2(const char* method, const char* url, const tsk_buffer_t* entity_body, const char* qop, tsk_md5string_t* ha2)
{
    int ret;
    /* RFC 2617 - 3.2.2.3 A2
...
/**@ingroup trtsp_auth_group
 *
 * Generates RTSP digest response as per RFC 2617 subclause 3.2.2.1.
 *
 * @param [in,out]	ha1			HA1 string generated using  @ref trtsp_auth_digest_HA1 or @ref trtsp_auth_digest_HA1sess.
 ...
 * @param [in,out]	ha2			HA2 string generated using @ref trtsp_auth_digest_HA2.
 * @param [in,out]	response	A pointer to the response.
 *
 * @return	Zero if succeed and non-zero error code otherwise.
 **/
int trtsp_auth_digest_response(const tsk_md5string_t *ha1, const char* nonce, const nonce_count_t noncecount, const char* cnonce,
 ...
 */
tsk_size_t trtsp_auth_ws_response(const char* key, trtsp_auth_ws_keystring_t* response)
{
    if (!key || !response) {
